...
const getImage = (timestamp, maxRetries = 3, retryDelay = 2000) => {
  ...
  return new Promise((resolve, reject) => {
    const tryFetch = (retryCount = 0) => {
      attempt(retryCount)
        .then(resolve)
        .catch(error => {
          if (retryCount < maxRetries) {
            console.log(`Retry attempt ${retryCount + 1} of ${maxRetries} after error: ${error.message}`);
            setTimeout(() => tryFetch(retryCount + 1), retryDelay);
          } else {
